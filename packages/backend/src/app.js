...
app.post('/api/tasks', (req, res) => {
  try {
    ...
    const pr = (priority === 'P1' || priority === 'P2' || priority === 'P3') ? priority : 'P3';
    const stmt = db.prepare('INSERT INTO tasks (title, description, due_date, priority) VALUES (?, ?, ?, ?)');
    const result = stmt.run(title, description || '', due_date || null, pr);
...
app.put('/api/tasks/:id', (req, res) => {
  try {
    ...
    const result = stmt.run(title, description || '', due_date || null, pr, req.params.id);
